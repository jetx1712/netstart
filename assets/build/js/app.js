...
$(document).on('click', '.qtdPersons', function () {
  let qtdPersons = $(this).val();

  $.ajax({
    url: url_atual + 'planoIdeal/fetch_ideal_plan_qtd_persons',
    method: 'POST',
    data: {
      qtdPersons: qtdPersons
    },
    success: function (data) {
      let dataObj = JSON.parse(data);
      $('.plan-inner-title').html(dataObj.title);
      $('.plan-result-link').attr('href', url_atual + 'contract/' + dataObj.url_friendly);
    }
  });
});
